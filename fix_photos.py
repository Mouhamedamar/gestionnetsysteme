...
for product in products_without_photo:
    ...
    if matching_images:
        ...
        print(f"    ✗ Aucune image trouvée pour l'ID {product.id}")

# 4. Afficher le résumé
print("\n" + "=" * 60)
print(f"✓ Produits mis à jour: {updated}")
all_with_photo = Product.objects.exclude(photo__isnull=True).exclude(photo="")
print(f"✓ Total produits avec photo: {all_with_photo.count()}")
print("=" * 60)
