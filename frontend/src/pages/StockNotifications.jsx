...
import { MessageSquare, ArrowLeft, Plus, Trash2, Send, Save, Search, AlertCircle, User, Phone, Mail, Settings2, Bell, Pencil, X, ExternalLink } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <div className="flex flex-wrap justify-end">
        <Link
          to="/stock-reminders"
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-primary-200 text-primary-700 font-medium text-sm hover:bg-primary-50 transition-colors"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-8 shadow-xl border-white/60">
            ...
            <form
              onSubmit={async (e) => {
                ...
                try {
                  const response = await apiCall('/api/stock-alert-settings/', {
                    method: 'PATCH',
                    body: JSON.stringify({ reminder_interval_days: Number(settings.reminder_interval_days) }),
                  });
                  if (response.ok) {
            ...
            >
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Me rappeler dans</label>
                <select
                  value={String(settings.reminder_interval_days ?? 0)}
                  onChange={(e) => setSettings({ ...settings, reminder_interval_days: Number(e.target.value) })}
              ...
              {settings.last_reminder_sent_at && (
                <p className="text-sm text-slate-500">
                  Dernier rappel envoyé le : {new Date(settings.last_reminder_sent_at).toLocaleString('fr-FR')}
                </p>
              )}
