...
import { User, LogOut, Bell, Search, Menu, Settings, Calendar } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { API_BASE_URL, GOOGLE_CALENDAR_EMBED_URL } from '../config';
...
const Header = ({ onOpenMobileMenu }) => {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-slate-200/60 bg-white/70 backdrop-blur-xl shadow-sm shadow-slate-200/50 safe-area-inset-top">
      <div className="flex items-center justify-between gap-2 sm:gap-4 px-3 sm:px-4 md:px-6 lg:px-8 py-3 min-h-[3.5rem] sm:min-h-[4rem]">
        ...
        <form
          ...
          className={`hidden md:flex items-center gap-3 flex-1 max-w-xl min-w-0 transition-all duration-300 rounded-2xl border px-4 py-2.5 ${
            searchFocused
        ...
        <div className="flex items-center gap-1 sm:gap-2 ml-auto shrink-0 flex-wrap min-w-0">
          {/* Calendrier — ouvre le calendrier Google dans un nouvel onglet */}
          <a
            href={GOOGLE_CALENDAR_EMBED_URL || 'https://calendar.google.com/calendar/'}
            target="_blank"
            rel="noopener noreferrer"
            className="p-2.5 rounded-xl text-slate-500 hover:text-primary-600 hover:bg-primary-50 transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 inline-flex items-center justify-center"
            title="Ouvrir le calendrier Google"
            aria-label="Ouvrir le calendrier Google"
          >
          ...
          {(user?.role === 'admin' || user?.is_staff) && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-2.5 rounded-xl text-slate-500 hover:text-primary-600 hover:bg-primary-50 transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2"
              title="Administration Django"
