...
const IconCircle = ({ icon: Icon, children }) => (
  <div className="flex items-start gap-2">
    ...
    <div className="text-white text-base font-semibold leading-tight">{children}</div>
  </div>
);
...
const DocumentFooter = ({ className = '', company = 'NETSYSTEME' }) => {
    ...
    return (
      <div className={`document-footer document-footer-sse ${className}`} style={{ pageBreakInside: 'avoid' }}>
        <div className="bg-[#ea580c] text-white px-4 py-3">
          ...
          <div className="flex flex-wrap gap-4 sm:gap-6 items-center text-base text-white border-t border-white/30 pt-2">
  ...
  return (
    <div className={`document-footer ${className}`} style={{ pageBreakInside: 'avoid' }}>
      ...
      <div className={`${style.band} text-white px-6 py-4`}>
        <p className="text-center font-bold tracking-[0.3em] text-lg uppercase">
          MERCI POUR VOTRE CONFIANCE
        </p>
      </div>

      {/* Zone à deux colonnes : blanche avec texte coloré (bleu) */}
      <div className={`flex bg-white ${style.text} text-lg px-6 py-4 gap-6 items-start`}>
        ...
        <div className="flex-1 space-y-2">
          <div className="flex items-start gap-2">
            <MapPin className="w-5 h-5 mt-0.5" aria-hidden />
            <span className="font-semibold leading-snug">
              Ouest foire sur la route de l&apos;aeroport Leopold Sedar Senghor immeuble Seigneurie
            </span>
          </div>
          <div className={`inline-block ${style.badge} text-white px-4 py-2 rounded text-base font-bold tracking-wide`}>
