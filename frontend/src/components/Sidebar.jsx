...
const Sidebar = ({ mobileMenuOpen = false, onCloseMobile, onCollapsedChange }) => {
  ...
  const adminMenuItems = [
    ...
    { path: '/clients/blacklist', label: 'Clients blacklistés', icon: Ban },
    { path: '/agrement', label: 'Agrément', icon: FileText },
    ...
    { path: '/stock-reminders', label: 'Rappels stock faible', icon: Bell },
  ];
  ...
  const getMenuItems = () => {
    ...
    switch (role) {
      case 'admin':
        defaultPermissions = ['/', '/prospects', '/products', '/stock', '/stock-movements', '/stock-notifications', '/stock-reminders', '/interventions',
          '/installations', '/clients', '/clients/blacklist', '/agrement', '/quotes', '/invoices',
          '/proforma-invoices', '/expenses', '/zone-de-travail', '/pointage', '/users'];
        baseMenuItems = adminMenuItems;
        break;
      case 'technicien':
        defaultPermissions = ['/', '/prospects', '/interventions', '/pointage'];
        baseMenuItems = technicienMenuItems;
        break;
      case 'commercial':
        defaultPermissions = ['/', '/prospects', '/clients', '/clients/blacklist', '/agrement', '/interventions', '/pointage'];
        baseMenuItems = commercialMenuItems;
        break;
      case 'pointage_only':
        defaultPermissions = ['/', '/pointage'];
        baseMenuItems = pointageOnlyMenuItems;
        break;
      default:
        defaultPermissions = ['/', '/prospects', '/products', '/stock', '/stock-movements', '/stock-notifications', '/stock-reminders', '/interventions',
          '/installations', '/clients', '/clients/blacklist', '/agrement', '/quotes', '/invoices',
  ...
  return (
    <aside
      className={`
        ...
        inset-y-0 left-0
        w-72 max-w-[min(85vw,320px)] lg:max-w-none
    ...
    >
      <div className="h-full flex flex-col bg-white/95 lg:bg-white/90 backdrop-blur-xl border-r border-slate-200/80 shadow-xl lg:shadow-lg shadow-slate-200/30 safe-area-inset-left">
        ...
        {/* Administration (modifier sans code) – admins uniquement */}
        {(role === 'admin' || user?.is_staff) && (
          <div className="flex-shrink-0 px-3 pb-3">
            <a
              ...
              className="nav-item group flex items-center gap-3 rounded-xl py-3 px-4 text-slate-500 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 min-w-0"
            >
              <ExternalLink className="w-5 h-5 flex-shrink-0" />
              {!collapsed && (
                <span className="font-medium text-sm truncate flex-1 min-w-0">
                  Administration (modifier sans code)
                </span>
              )}
            </a>
          </div>
